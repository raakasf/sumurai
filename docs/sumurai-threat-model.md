...
| Redis `{jti}_synced_transactions`      | Authenticated API call in a session      | Authenticated user → Backend → Redis | Scoped to JWT `jti`; other sessions keep their blobs until TTL | `backend/src/services/cache_service.rs` (`synced_transactions_key`)                                            |
...
3. **Cross-session transaction cache wipe (mitigated for cross-tenant):** Previously a global Redis key allowed any authenticated session to clear everyone’s cached transaction list. Reads/writes/deletes now use `{jti}_synced_transactions`, so one session cannot wipe another’s blob via this mechanism. Residual: a new `jti` after refresh starts with an empty cache until the next sync.
...
| TM-002    | Authenticated attacker (same vs other sessions) | Valid registered account                                                                 | Invoke `clear_transactions(jti)` / `add_transaction(jti, …)` or logout (`clear_jwt_scoped_data` also matches `{jti}*`) | Other sessions’ cached transaction blobs are not cleared; only the caller’s `jti` key is affected | Redis cache, transaction data availability               | JWT+Redis session; cache key `synced_transactions_key(jti)` → `{jti}_synced_transactions` (`backend/src/services/cache_service.rs`); logout clears all keys prefixed with that `jti`                                                                                                                                       | Residual: stable `user_id` is not the cache scope—intentionally `jti` per product choice; token refresh mints a new `jti` and cache does not carry over until next sync                                                                 | Ops: optional one-time `DEL synced_transactions` for legacy global key post-deploy; monitor abnormal sync frequency per session                                                                                                         | Alert on rapid repeated sync calls from a single user; monitor cache miss rate spikes                          | Low        | Medium          | **Medium**   |
...
| TM-010    | Operator / deployment error         | Production deploy before trusted nginx certificate issuance or renewal                         | Missing, self-signed, or soon-expiring certificate material would otherwise reach users       | Browser trust warnings; MitM possible if users bypass warning                                      | User sessions in transit                                 | Production compose override sets `ENVIRONMENT=production`, public 80/443 ports, and `SSL_PORT=443`; nginx startup rejects missing, self-signed, and certs expiring within 14 days; nginx healthcheck validates active certificate state; production TLS runbook documents issuance and renewal | Operator must still run initial ACME issuance and schedule renewal; runtime container smoke verification remains deployment responsibility                                                          | Use `docs/PRODUCTION_TLS.md`; run certbot issuance before production start; schedule `certbot renew`; keep certbot volumes persistent; verify certificate chain and nginx health after deploy          | `TLS_PROVISIONING_ERROR` logs; nginx unhealthy status; external cert-expiry monitoring before the 14-day window                        | Low        | Medium          | **Low**    |
