...
- Is the `ENCRYPTION_KEY` env var enforced as a required secret in deployment config, or is the fallback path reachable in staging?
...
- **Nginx → Seq UI and OTLP (HTTP, Docker internal; no IP restriction)**
  - Data: structured logs/traces (contain email PII in `auth.username` attribute), Seq admin UI
...
| Browser OTLP exporter                  | Browser sends spans to `/ingest/otlp`      | Browser → Nginx → Seq                | `auth.username` (email) in span attributes        | `frontend/src/services/authService.ts:54`                     |
...
| `ENCRYPTION_KEY` hardcoded fallback    | Missing env var at deploy time             | Operator misconfiguration            | AES key becomes well-known default                | `backend/src/services/repository_service.rs:102`              |
...
4. **Misconfigured deployment → Teller token mass decryption:** Operator deploys without setting `ENCRYPTION_KEY` → backend silently falls back to hardcoded key → attacker who later obtains a DB dump decrypts all `encrypted_access_token` BYTEA values using the known fallback key → Teller API access for all users.
...
6. **Seq UI data exposure:** Seq UI at `/seq/` is publicly proxied without IP restriction. If Seq auth is not configured or is weak, an attacker browses all structured logs and traces including email PII in `auth.username` span attributes and account IDs.
...
| TM-006    | Operator misconfiguration           | `ENCRYPTION_KEY` env var absent at deploy                | Backend uses hardcoded fallback AES-256-GCM key; all Teller tokens encrypted with known key  | All Teller access tokens in DB decryptable if DB is dumped                                         | Teller access tokens, `ENCRYPTION_KEY`                   | AES-256-GCM encryption in use (`backend/src/services/repository_service.rs:125`)            | Fallback to hardcoded key instead of hard failure (`repository_service.rs:102`); key absent from required vars in `.env.example`                                                                 | Panic/exit on missing `ENCRYPTION_KEY` (same pattern as `JWT_SECRET`); add to required vars; emit a startup warning alertable in Seq if fallback is used                                               | Startup log with distinct "using fallback encryption key" message; alert on it                                 | Low        | Critical        | **High**     |
...
| TM-009    | Internal logging pipeline           | Any authenticated login                                  | Email PII flows into OTLP spans (`auth.username` attribute) and backend logs on auth failure | GDPR/CCPA compliance risk; PII in potentially public Seq endpoint                                  | Email PII, Seq telemetry                                 | `NEXT_PUBLIC_OTEL_SANITIZE_HEADERS` flag exists; encrypted token hash used in backend spans | `auth.username: credentials.email` explicitly set on login span (`frontend/src/services/authService.ts:54`); email logged in backend on register/login failure (`backend/src/main.rs:~490,~567`) | Replace `auth.username` with a pseudonymous hash of the user ID; redact email from backend auth failure logs; enforce sanitize flags in production compose                                             | Audit Seq for PII fields on a schedule                                                                         | High       | Medium          | **Medium**   |
...
- **High:** Cross-tenant data access requiring only a valid account; XSS-to-token-theft path; silent misconfiguration leading to decryptable Teller tokens; publicly exposed admin UI containing PII.
...
| `backend/src/services/repository_service.rs` (lines 100–110) | Hardcoded `ENCRYPTION_KEY` fallback; AES-256-GCM key management                        | TM-006             |
...
| `frontend/src/services/authService.ts`                       | `auth.username: email` PII in OTLP login span                                          | TM-009             |
